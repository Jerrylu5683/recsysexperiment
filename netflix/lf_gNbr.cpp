...
namespace svd{
    ...
    vector< double> simLevel(ITEM_NUM+1);    //用以k个邻居下的相似度阀值
    ...
    void model(int dim, double & alpha, double & beta)
    {
            ...
            for( u = 1; u < USER_NUM+1; ++u) {   //循环处理每一个用户 
                for(i = 0; i < userItems[u].size(); ++i) {  // 循环处理u打分过的每一个item
                	...
                	for(j=0; j < userItems[u].size(); ++j) {  
                		if((sim[userItems[u][i]][userItems[u][j]]) < simLevel[userItems[u][i]])
                		{
                	...
                	for(j=0; j < userItems[u].size(); ++j) {
                		if((sim[userItems[u][i]][userItems[u][j]]) < simLevel[userItems[u][i]])continue;
                		double buj = mean + bu[u] + bi[userItems[u][j]];
   	...
   	{	
	    ...
	    while(from.getline(rateStr,15138)){
	        ...
	        simLevel[lineNum] = getKmax(sim[lineNum],K_NUM); //获取k个邻居的相似度阀值
	        /*
	        qsort(sim[lineNum],1,sim[lineNum].size()-1);
	        for(int i=K_NUM+1; i<sim[lineNum].size();) {
	        	sim[lineNum].erase(sim[lineNum].begin()+i);
        	}
        	//cout<<sim[lineNum].size()<<endl;
        	//exit(0);
        	*/
        	++lineNum;
	    }
