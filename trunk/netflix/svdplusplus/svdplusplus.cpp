...
*/
/**
 * 本程序的目的是实现koren在SIGKDD'08论文中的方法，svd++的方法
...
namespace svd{
    ...
    double puTemp[USER_NUM+1][K_NUM+1] = {0};   //用于存储用户的属性描述p
            ...
            for( u = 1; u < USER_NUM+1; ++u) {   //循环处理每一个用户 
                ...
                for(i=0; i < RuNum; ++i) {// 循环处理u打分过的每一个item
                	...
                	for( k=1; k< K_NUM+1; ++k) {
	            		...
	            		q[itemI][k] += alpha * (eui*puTemp[u][k] - beta*q[itemI][k]);
                ...
                for(j=0; j < RuNum; ++j) {// 循环处理u打分过的每一个item
	            	...
	            	for( k=1; k< K_NUM+1; ++k) {
	            		y[itemJ][k] += alpha * (sqrtRuNum * sumQE[k] - beta*y[itemJ][k]);
	            		//y[itemJ][k] += alpha * (sumQE[k] - beta*y[itemJ][k]);
	            	}
            	}
            }
            nowRmse =  sqrt( rmse / n);
    ...
    double predictRate(int user,int item)
    {
    	...
    	if(RuNum > 1)
    	{
    		ret = mean + bu[user] + bi[item] +  dot(puTemp[user],q[item]);//这里先不对k进行变化，先取k=无穷大
    	}
