 ...
 * （4）use the formula （4.12） in the page 125 of Recommender system handbook to calculate the similarity of
...
namespace knn{
    ...
    void loadRating(char * filePath, map<int,short> rateMatrixLocal[],const char* separator)
    {
        ...
        while(from.getline(rateStr,256)){
            ...
            if (0 == rate || -1 == rate) { //if the rate is ilegal, then discard it
                continue;
            }
